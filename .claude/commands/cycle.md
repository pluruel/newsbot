이 커맨드는 `run_cycle.py`가 슬롯·카테고리를 채워서 부르는 내부용이다. 입력파일·시장 스냅샷·guids는 그쪽이 미리 만들어 두므로, 인자 없이 이 커맨드를 실행하면 읽을 입력파일이 없어 실패한다.

`$ARGUMENTS`가 비어 있으면 slot·category를 되묻지 말고 실행도 하지 마라. 대신 `start_job("cycle")`을 호출한다 — 현재 시각(KST) 슬롯으로 tech·markets 양쪽을 자동으로 돈다. start_job 도구가 없는 세션이면 사용자에게 그 사실만 알린다.
