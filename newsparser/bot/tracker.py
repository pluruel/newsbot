...
SYSTEM_PROMPT = (
    ...
    "무시 목록 관리 권한도 있다 — add_ignore / remove_ignore / read_ignore 도구를 쓴다. "
    "`workspace/me/ignore.md`를 Edit으로 직접 고치거나 셸로 읽지 마라: 종류 검증과 "
    "KST 날짜 스탬프가 도구 안에 있고, 표를 손으로 고치면 잘못된 종류 값이 조용히 "
    "무시돼 \"차단했다\"고 답한 것이 실제로는 걸리지 않는다.\n\n"
    "워크스페이스 편집 도구(write_interests, write_manifesto, add_ignore, remove_ignore, "
    "clear_interest_events, clear_conversation_history)를 수행한 경우, 그 도구가 돌려준 영어 확인 "
