...
def _fake_run_claude_writes_report_sync(prompt, **kw):
    """Writes the report file synchronously (for use inside async helpers)."""
    import os as _os
...
async def _fake_run_claude_writes_report(prompt, **kw):
    _fake_run_claude_writes_report_sync(prompt, **kw)
    return RunResult(text="")


async def test_run_cycle_writes_guids_file_before_calling_run_claude(tmp_path):
    ...
    with patch("newsparser.scripts.run_cycle.run_claude", side_effect=fake_claude), \
         ...
         patch("newsparser.scripts.run_cycle.classify_article", AsyncMock(return_value="tech")), \
         patch("newsparser.scripts.run_cycle.send_long_message"):
        await script.main("2026-05-08-12")

    assert guids_seen == [True], "guids file must exist when run_claude is called"


async def test_run_cycle_sends_digest_to_telegram(tmp_path):
    insert_article("g1", "src", "t1", "u1", None, "body", category="tech")
...
async def test_run_cycle_category_error_doesnt_stop_other(tmp_path):
    insert_article("g1", "src", "t1", "u1", None, "body", category="tech")
    ...
    with patch("newsparser.scripts.run_cycle.run_claude", side_effect=fake_claude), \
         patch("newsparser.scripts.run_cycle.build_input_file"), \
         patch("newsparser.scripts.run_cycle.classify_article", AsyncMock(return_value="markets")), \
         patch("newsparser.scripts.run_cycle.send_long_message", side_effect=lambda m: sent.append(m)):
        await script.main("2026-05-08-12")
...
async def test_snapshot_block_prepended_to_input(tmp_path, monkeypatch):
    ...
    with patch.object(run_cycle, "run_claude", side_effect=fake_run_claude), \
        ...
        await run_cycle.main("2026-05-09-12")
